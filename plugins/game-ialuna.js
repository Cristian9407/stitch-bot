 ...
 function shouldProcessMessage(msg, rawText) {
  ...
  function getFallbackResponse(text, isPrivate = false) {
    ...
    if (greetings.some(greeting => lowerText.includes(greeting))) {
      return '🌙 ¡Hola! Soy Stitch-Botv6, tu asistente inteligente creado por Cristian. ¿En qué puedo ayudarte hoy? ✨';
    }
    ...
    if (whoAreYou.some(phrase => lowerText.includes(phrase))) {
      return '🌙 Soy Stitch-Botv6, un asistente inteligente creado por Cristian. Puedo ayudarte con muchas cosas como generar imágenes, descargar música, mostrar el clima, información de fútbol y mucho más. ¿En qué te puedo ayudar? ✨';
    }
    ...
    return isPrivate  
  ? '🌙 Hola, soy Stitch-Botv6. Mi sistema de IA está temporalmente fuera de línea, pero aún puedo ayudarte con:\n\n• 🎵 Descargar música (solo dime el nombre)\n• 🖼️ Generar imágenes\n• 🌤️ Información del clima\n• ⚽ Información de fútbol argentino\n• 📋 Mostrar el menú de comandos\n• 🌐 Mis redes sociales\n• 🔒 Abrir o cerrar el grupo\n\n¿Qué necesitas?'
  : '🌙 Hola, soy Stitch-Botv6. Mi IA está temporalmente offline, pero puedo ayudarte con música, imágenes, clima, fútbol y abrir/cerrar grupo. ¿Qué necesitas?';
  }

  async function callGeminiAPI(text, isImagePrompt = false, isPrivate = false) {
    try {
      const systemPrompt = isImagePrompt 
        ? "Eres Stitch-Botv6, un asistente especializado en crear prompts detallados para generacion de imagenes. Tu creador es German Miño. Mejora el prompt del usuario haciendolo mas descriptivo y especifico."
        : isPrivate 
          ? "Eres Stitch-Botv6, un asistente inteligente, amigable y profesional creado por Cristian. Mantén conversaciones naturales y útiles. Si te preguntan sobre tu creador, desarrollador, programador o quien te hizo, responde que fuiste creado por German Miño. Siempre recuerda que eres Luna-Botv6-Project. También puedes ayudar con información del clima y fútbol argentino."
          : "Eres Stitch-Botv6, un asistente inteligente, amigable y profesional creado por Cristian. Si te preguntan sobre tu creador, desarrollador, programador o quien te hizo, responde que fuiste creado por German Miño. Responde de manera útil y concisa cuando te mencionen. También puedes ayudar con clima y fútbol argentino.";
