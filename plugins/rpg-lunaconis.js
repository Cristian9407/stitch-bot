...
const handler = async (m, { conn }) => {
  ...
  if (Math.random() < 0.1) {
    cooldowns[id] = now
    saveCooldowns()
    return m.reply('💥 Tu pico se rompió y no lograste obtener ninguna StitchCoin...')
  }
  ...
  return m.reply(
    ...
    `🔹 Ganaste: *${lunaCoinsGanadas}* StitchCoins\n` +
    `✨ Experiencia: +${expGanada} EXP\n` +
    `🪙 Total actual de StitchCoins: ${getUserStats(id).lunaCoins}`
  )
}

handler.help = ['minar']
handler.tags = ['juegos']
handler.command = /^minarstitch$/i
